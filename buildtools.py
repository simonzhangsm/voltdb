...
class BuildContext:
    def __init__(self, args):
        ...
        self.OBJ_INCLUDE_DIRS = ["3pty-install/include", "3pty-install/include/s2"]
    ...
    def getOpenSSLToken(self):
        if self.PLATFORM == 'Darwin':
            return "./Configure darwin64-x86_64-cc"
...
def buildThirdPartyTools(CTX, makefile):
    ...
    makefile.write('build-third-party-tools: build-s2-geometry build-pcre2\n')
    makefile.write('\n')
    ...
    makefile.write('OPENSSL_NAME=openssl-${OPENSSL_VERSION}\n')
    makefile.write('OPENSSL_SRC=${THIRD_PARTY_SRC}/${OPENSSL_NAME}\n')
    makefile.write('OPENSSL_OBJ=${OBJDIR}/${OPENSSL_NAME}\n')
    makefile.write('OPENSSL_INSTALL=${INSTALL_DIR}\n')
    makefile.write('OPENSSL_TARBALL=${TARBALLS_DIR}/${OPENSSL_NAME}.tar.gz\n')
    makefile.write('#\n')

    makefile.write("# These are google S2 library's source and object directories.\n")
    makefile.write("#\n")
    makefile.write('GOOGLE_S2_NAME=s2geometry\n')
    makefile.write('GOOGLE_S2_SRC=${THIRD_PARTY_SRC}/${GOOGLE_S2_NAME}\n')
    makefile.write('GOOGLE_S2_OBJ=${OBJDIR}/${GOOGLE_S2_NAME}\n')
    ...
    makefile.write('PCRE2_VERSION=%s\n' % CTX.PCRE2_VERSION)
    ...
    makefile.write('\t@echo "PCRE2_SRC         = $(PCRE2_SRC)"\n')
    makefile.write('\t@echo "PCRE2_INSTALL     = $(PCRE2_INSTALL)"\n')
    ...
    makefile.write('########################################################################\n')
    makefile.write('.PHONY: build-openssl \n')
    makefile.write('\n')
    makefile.write('# OpenSSL\n')
    makefile.write('build-openssl: configure-openssl\n')
    makefile.write('\t  @echo Building OPENSSL ; \n')
    makefile.write('\t  (cd "$(OPENSSL_OBJ)"; make; make INSTALLTOP=\"$(OPENSSL_INSTALL)\" OPENSSLDIR=\"$(OPENSSL_INSTALL)\" install );  \n')
    makefile.write('\n')

    makefile.write('configure-openssl: unpack-openssl \n')
    makefile.write('\t @echo Configuring OPENSSL; \n')
    makefile.write('\t (cd "${OPENSSL_OBJ}"; \\\n')
    makefile.write('\t if [ ! -f Makefile ] || [ ! -f "${OPENSSL_INSTALL}/lib/libcrypto.a" ] || [ ! -d "${OPENSSL_INSTALL}/include/openssl" ] ; then \\\n')    
    makefile.write('\t   rm -rf "${OPENSSL_INSTALL}/include/openssl" "${OPENSSL_INSTALL}/lib/libcrypto.a" "${OPENSSL_INSTALL}/lib/libssl.a"; \\\n')
    makefile.write('\t   %s; \\\n' % CTX.getOpenSSLToken() )
    makefile.write('\t fi)\n')
    makefile.write('\n')

    makefile.write('unpack-openssl: \n')
    makefile.write('\tif [ ! -d "${OPENSSL_OBJ}" ] ; then \\\n')
    makefile.write('\t  tar -xzvf "${OPENSSL_TARBALL}" -C "${OBJDIR}"; \\\n')
    makefile.write('\tfi\n')
    makefile.write('\n')
    ...
    makefile.write('\t    @echo Building S2 Geometry Library ;\n')
    makefile.write('\t    (cd "${GOOGLE_S2_OBJ}"; make; make install); \n')
    makefile.write('\n')
    ...
    makefile.write('configure-s2-geometry: unpack-s2-geometry build-openssl \n')
    makefile.write('\t@echo Configuring S2 Geometry Library for building.; \n')
    makefile.write("\tif [ -d ${GOOGLE_S2_OBJ} ] ; then \\\n")
    makefile.write('\t    (cd "${GOOGLE_S2_OBJ}"; \\\n')
    makefile.write('\t    cmake -DCXX_VERSION_FLAG=\"%s\" -DCMAKE_INSTALL_PREFIX=\"${GOOGLE_S2_INSTALL}\" -DOPENSSL_INCLUDE_DIR=\"${OPENSSL_INSTALL}/include\" -DOPENSSL_LIBRARIES=\"${OPENSSL_INSTALL}/lib/libcrypto.a\" -DCMAKE_BUILD_TYPE=\"${S2_BUILD_TYPE}\" . ); \\\n' % CTX.CXX_VERSION_FLAG )
    makefile.write("\tfi\n")
    makefile.write('\n')
    
    makefile.write('unpack-s2-geometry:\n')
    makefile.write('\tif [ ! -d "${GOOGLE_S2_OBJ}" ] ; then \\\n')
    makefile.write('\t  cp -r "${GOOGLE_S2_SRC}" "${OBJDIR}"; \\\n')
    makefile.write('\tfi\n')
    makefile.write('\n')
...
def buildMakefile(CTX):
    ...
    makefile.write("########################################################################\n")
    makefile.write("\n")
    makefile.write("clean: clean-3pty-install clean-generated-tests\n")
